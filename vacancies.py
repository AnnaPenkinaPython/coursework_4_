...
def sorting_salary(vacancies: list, key_search: str) -> list:
    """Сортирует список вакансий по зарплате, где key_search может быть зарплата ОТ и зарплата ДО"""
    sorted_by_salary = sorted(vacancies, key=lambda x: x.get(key_search), reverse=True)
